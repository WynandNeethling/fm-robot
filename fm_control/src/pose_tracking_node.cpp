...
int main(int argc, char** argv)
{
  ...
  {
    RCLCPP_FATAL(logger, "Timed out waiting for a current robot state on '%s'.",
  ...
  RCLCPP_INFO(logger, "pose_tracking_node ready; servoing the EE to /target_pose (frame '%s').",
  ...
  while (rclcpp::ok())
  {
    const moveit_servo::PoseTrackingStatusCode rc = tracker.moveToPose(lin_tol, ang_tol, target_timeout);
